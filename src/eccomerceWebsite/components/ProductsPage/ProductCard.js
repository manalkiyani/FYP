...
const ProductCard = ({productIds }) => {
 ...
 const openproductDeletion = (id) => {
  ...
  const delProduct = async () => {
    ...
    axios
      .delete(`http://localhost:8800/api/products/delproduct/${ProductDeleteId}`)
      .then(function (response) {
        ...
        setTemplate({
          ...
          data: {
            products: products.filter((productId) => productId !== ProductDeleteId),
          },
        });

        toast.success("Product Deleted Successfully");
        setProducts(products.filter((product) => product._id !== ProductDeleteId));
  ...
  return (
    ...
    {console.log("this is product card " + productIds)}
     <ConfirmDeleteDialog open={productDeletion} onClose ={closeproductDeletion} onConfirm={delProduct} />
