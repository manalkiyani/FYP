...
const CartPage = (props) => {   //we have to receive userid here in the props to make this code work
    ...
    const incrementQuantity = async(pid) => {
        let temp = cartProducts.filter(element => {
            if(element._id == pid){
                ...
                console.log("quantity of "+element.name+" is now "+ element.quantity)
                return element;
    ...
    const response = await axios.put("http://localhost:8800/api/products/editquantityincart", {
  ...
  console.log("this is response "+response.status)
    ...
    useEffect(() => {
        const fetchCartProducts = async () => {
          try {
            const response = await fetch("http://localhost:8800/api/products/getfromcart/", {
              method: "POST",
              headers: {
            ...
            const productRequests = cart.map((product) => {
              return fetch(`http://localhost:8800/api/products/getproduct/${product.productId}`)
                .then((res) => res.json());
            });
    
            const products = await Promise.all(productRequests);
    
            setCartProducts(cart.map((product, index) => {
      ...
      return (
        <>
                    ...
                    <tbody>
                        {cartProducts.map((product) => (
                            <tr key={product.productId} className={styles.row}>
                                ...
                                <td className={styles.cell}>
                                    <span className={styles.icons} onClick={() => decrementQuantity(product.productId)}>-</span>
                                    {product.quantity}
                                    <span className={styles.icons} onClick={() => incrementQuantity(product.productId)}>+</span>
                                </td>
                                <td className={styles.cell}>${product.price * product.quantity}</td>
                                <td className={styles.cell}>
                                    <span style={{color:'red',fontWeight:'bold'}} onClick={() => removeProductFromCart(product.productId)}>X</span>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            { open && <CheckOutForm userid = {'63e8df1974cc16f2b7ecacb6'} removeAllFromCartAfterOrderReceived={removeAllFromCartAfterOrderReceived} subtotal={subtotal} cartProducts={cartProducts} setCartProducts={setCartProducts} open={open} setOpen={setOpen}></CheckOutForm>
