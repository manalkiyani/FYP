...
function AdminDasboard(){
    ...
    useEffect(() => {
        ...
        axios.get("http://localhost:8800/api/admin/registeredadmins")
        .then((response) => {
          setCount(response.data.count);
        });
        

         axios.get("http://localhost:8800/api/admin/getadminsdata")
        .then(res => {
        ...
        axios.get("http://localhost:8800/api/admin/gettotalpaymentsandmessages")
        .then(res => {
    ...
    return(
        <div>
            ...
            <div style={{backgroundColor:'#EBF1FC',paddingTop:'25px', paddingBottom:'25px', width:'80%', marginLeft:'10%'}}>
            <div style={{display:'flex', justifyContent:'center'}}>            
            <Card className={classes.background1} tagline="Registered Users"> {count}</Card>
            <Card  className={classes.background2} tagline="Live Websites"> {totalLiveWebs}</Card>
            <Card className={classes.background3} tagline="Payments"> {totalPayments}</Card>
            <Card className={classes.background4} tagline="Messages"> {totalMessages}</Card>
        </div>
        ...
        <TableBody>
          {adminsdata && adminsdata.map((row,index) => {
            ...
            <TableRow
              ...
              <TableCell align="center">{row.savedTemplatesCount}</TableCell>
